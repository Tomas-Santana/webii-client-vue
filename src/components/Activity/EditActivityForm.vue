...
const { handleSubmit, setFieldValue, values } = useForm({
  ...
  initialValues: {
    name: props.currentActivity.activity_name,
    estimatedHours: props.currentActivity.estimated_hours,
    deadline: new Date(props.currentActivity.deadline).toISOString().split('T')[0],
  },
})
...
const onSubmit = handleSubmit(async (values) => {
  ...
  const response = await ProjectModule.ActivityManager.editActivity(
    props.currentActivity.activity_id,
    values.name,
    values.deadline,
