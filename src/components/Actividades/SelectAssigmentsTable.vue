...
const createAssignment = async () => {
  if (!props.activityId) {
    toast.error("No se ha creado una actividad")
    return
  }
  ...
  for (const [key, _] of Object.entries(rowSelection.value)) {
    const member = props.data[Number(key)]
    const res = await ProjectModule.AssignmentManager.insertAssignment(member.id, props.activityId)
...
<template>
  <div class="flex flex-col gap-4">
    ...
    <div>
      <Button @click="createAssignment" :variant="Object.entries(rowSelection).length > 0 ? 'default' : 'secondary'">
        {{
          Object.entries(rowSelection).length > 0 ? "Asignar" : "Saltar"
        }}
      </Button>
    </div>
  </div>
